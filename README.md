...
|5|直接清除数据缺失，不处理重复样本，特征工程2(时间特征1-5阶)，特征工程3-4(单5阶)|41.45/2.39|0.26800|2020.11.16|v|

注：这里进行特征工程2<br>
**总结**：引入差分五阶特征后，应该是产生了过拟合现象。也可能是其他特征的不平滑造成，给机器学习模型带来噪声<br>
**思考**：采用格子搜索的方法去寻找最优的减轻过拟合的参数，而这里找到的最优很可能会是过拟合的最优。为什么引入甚至少量的差分特征会导致精度的大幅降低。
